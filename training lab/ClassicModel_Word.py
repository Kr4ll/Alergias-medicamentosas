...
# Library to balance the difference in samples
...
ruta_datos_alergico = "../data/ByPatients/Alergico"
...
X = vectorizador.fit_transform(textos_preprocesados)
...
# save vectorizator TF-IDF
tokenizer_file_path = os.path.join("../models/Classic_models", f"tokenizer.joblib")
joblib.dump(vectorizador, tokenizer_file_path)
...
for model in tqdm(models):
    ...
    model_file_path = os.path.join("../models/Classic_models", f"{counter}{model_name}.joblib")
    ...
    report_file_path = os.path.join("../results", f"{model_name}{counter}_classification_report.txt")
    ...
    if hasattr(model, 'coef_'):  # SVC with linear kernel
        with open(report_file_path, 'a') as report_file:
            ...
            top_features = np.argsort(coef)[-10:]  # Get top 10 features
            report_file.write("\n")
            report_file.write(f"Palabras con mas influencia en el modelo  {model_name}: \n")
            for i in top_features:
                report_file.write(f"{feature_names[i]}: {coef[i]} \n")
        ...
        with open(report_file_path, 'a') as report_file:
            ...
            report_file.write(f"Palabras con mas influencia en el modelo {model_name}: \n")
            for i in top_features:
                report_file.write(f"{feature_names[i]}: {importances[i]} \n")
